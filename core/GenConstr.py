...
def GenConstr(model, res, node, p):
    ...
    if p == 0:
        ...
        if isLeaf(node):
            ...
            if node.val == 're':
                reserve = model[1]
                constr.append([Or([And(res[node.id][i + 1] == 1, reserve[i + 1][q - 1] == 1) for i in range(size)])])
            elif node.val == 'cl':
                claim = model[2]
                constr.append([Or([And(res[node.id][i + 1] == 1, claim[i + 1][q - 1] == 1) for i in range(size)])])
            elif node.val == 'free':
                free = model[3]
