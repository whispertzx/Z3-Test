...
# 针对叶子节点的约束，依次对应即可
# 基本约束，原子公式区域为1
formula_rz1 = [Implies(result[z][0] == 1, And([Implies(And(result[z][j+1] == 1, i != j), result[z][i+1] == 0)
                                               for i in range(4) for j in range(4)]))
               for z in [3, 4, 5, 6]]
# 对于节点4的约束，节点4为re(101)
formula_rz2 = [Implies(result[3][0] == 1, Or([And(result[3][i+1] == 1, re[i+1][101-101] == 1) for i in range(4)]))]
# 对于节点5的约束，节点5为cl(102)
formula_rz3 = [Implies(result[4][0] == 1, Or([And(result[4][i+1] == 1, cl[i+1][102-101] == 1) for i in range(4)]))]
# 对于节点6的约束，节点6为re(101)
formula_rz4 = [Implies(result[5][0] == 1, Or([And(result[5][i+1] == 1, re[i+1][101-101] == 1) for i in range(4)]))]
# 对于节点7的约束，节点7为re(102)
formula_rz5 = [Implies(result[6][0] == 1, Or([And(result[6][i+1] == 1, re[i+1][102-101] == 1) for i in range(4)]))]
...
# 初始约束，判断的区域范围
formula_pre = [And(result[0][0] == 1, result[0][1] == 0, result[0][2] == 1, result[0][3] == 1, result[0][4] == 1)]
...
formula_const1 = [And(Implies(result[i][0] == 0, And(result[2*(i+1)-1][0] == 0, result[2*(i+1)][0] == 0)),
                      Implies(result[i][0] == 1, And(And(result[2*(i+1)-1][0] == 1, And([Implies(result[i][j+1] == 1, Or(result[2*(i+1)-1][j+1] == 1, result[2*(i+1)-1][j+1] == 0)) for j in range(4)])),
                                                    And(result[2*(i+1)][0] == 1, And([Implies(result[i][j+1] == 1, Or(result[2*(i+1)][j+1] == 1, result[2*(i+1)][j+1] == 0)) for j in range(4)]))))
                      ) for i in [0]]
...
formula_const2 = [And(Implies(result[i][0] == 0, And(result[2*(i+1)-1][0] == 0, result[2*(i+1)][0] == 0)),
                      Implies(result[i][0] == 1, And(result[2*(i+1)-1][0] == 1, result[2*(i+1)][0] == 1,
                                                     And([Implies(result[i][j+1] == 1, Or(And(result[2*(i+1)-1][j+1] == 1, result[2*(i+1)][j+1] == 0),
                                                      ...
                                                      for j in range(4)]))))
                  for i in [1, 2]]
# formula_const2 = [And([Implies(result[i][j] == 1, Or(
#                 And(result[2*(i+1)-1][j] == 1, result[2*(i+1)][j] == 0),
#                 And(result[2*(i+1)-1][j] == 0, result[2*(i+1)][j] == 1))) for i in [1, 2] for j in range(4)])]
...
formula_s_x1 = [Implies(result[1][0] == 1, Or([And(result[3][i+1] == 1, result[4][j+1] == 1, r[i+1][j] == 7) for j in range(4) for i in range(4)]))]
# 对于节点3的空间约束，r[i+1][j] == 8表示EN
formula_s_x2 = [Implies(result[2][0] == 1, Or([And(result[5][i+1] == 1, result[6][j+1] == 1, r[i+1][j] == 8) for j in range(4) for i in range(4)]))]
...
# 下放约束3
formula_const3 = [And([Implies(result[i][j+1] == 0, And(result[2*(i+1)-1][j+1] == 0, result[2*(i+1)][j+1] == 0))
                      for i in range(3) for j in range(4)])]

con_const = formula_pre + formula_const0 + formula_const3 + formula_const2 + formula_s
